...
const CHECKOUT_NEXT_PAGE_SLUG = "4/order/en/au/upsell1b";
...
async function createOrderViaWallet(confirmationToken, paymentMethodId) {
  ...
  const orderData = {
    pageId: "m1jk4kwuwLn4Aav8c_6FgvAVOsS_uHyGI3sw4WhCBeK4cuAju0yab1podOFwM6hy",
    action: "process",
...
const getStates = async (countryIso2Code) => {
  ...
  try {
    if (!countriesStatesCache) {
      const res = await fetch('/69fbf0149099275155853b00-preview/scripts/countriesData/countriesStates.json', { cache: 'no-store' });
...
if (typeof validateAndSendToKlaviyo === "function") {
  try {
    ...
    var pageReadyPayload = {
      ...
      data: { pageName: "ADT-AUS-Checkout-V1", pageType: "Checkout" },
    };
